...
0. **Spike S0：Doubao embedding 维度验证**（**M0 第 0 步，阻塞迁移**）
   - 写一个一次性 `cmd/embedprobe/main.go`，复用 `llmproviders` 配置调 Doubao `doubao-embedding-text-240715`，输入一段中文字符串
   - 打印返回向量长度，作为 pgvector 列维度的**唯一权威依据**
   - 结果回填到本设计文档脚注 + `shared/embedding/doubao.go` 包级常量 `DoubaoEmbeddingDim`
...
1. **数据库迁移**（api 服务，新增 goose）—— **依赖 S0 结论**
   ...
   - 建 `kb_chunks(id, kb_name TEXT, document_ref TEXT, ordinal INT, text TEXT, token_count INT, embedding vector(<S0 验证的维度>), metadata_json JSONB, created_at)` —— M0 用一张表跑通；M1 阶段再拆出 `knowledge_bases` / `kb_documents` 等正式 schema
...
| 决策 | 选定 | 备选/为什么不选 |
|------|------|----------------|
| **Embedding 模型** | Doubao `doubao-embedding-text-240715`，**维度待 S0 probe 确认**（候选记忆值 2560，未验证） | OpenAI 3-small (1536) 需代理；本地 bge 部署成本重；Doubao 国内母语 + 已有 provider 集成 |
...
| 风险 | 缓解 | 触发 spike |
|------|------|------|
| Doubao 实际维度与记忆值不符 → 迁移返工 | **S0 必做**：写 cmd/embedprobe 跑一次拿真实维度再迁移 | S0 |
...
2. `kb_chunks.embedding` 维度从 1536 改 2560（Doubao）—— **review 后修正**：2560 改为"待 S0 probe 确认"，PRD 中 `vector(2560)` 暂保留但加 inline 注释
...
5. 锁定 Doubao 作为 embedding 默认（PRD `Embedder` 模块说明里更新默认 provider；具体维度待 S0）
...
2. **Doubao 维度**：迁移前必须先跑 Spike S0（`cmd/embedprobe`）拿真实维度。**禁止**凭记忆值 2560 写 DDL。S0 顺带 probe QPS、batch 上限、latency。
