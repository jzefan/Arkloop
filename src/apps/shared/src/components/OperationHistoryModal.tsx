...
import { X, Loader2, CheckCircle2, XCircle, Trash2 } from 'lucide-react'
import { useOperations, type OperationRecord, type ModuleAction } from './OperationContext'
...
function StatusBadge({ status, error }: { status: OperationRecord['status']; error?: string }) {
  switch (status) {
    case 'running':
      return (
      ...
      >
        ...
        >
          <div className="flex items-center gap-2 px-4 py-4">
            <span className="text-sm font-semibold text-[var(--c-text-heading)]">Operations</span>
            {operations.length > 0 && (
              ...
              {operations.map((op) => (
                <button
                  ...
                  className={[
                    ...
                    selectedId === op.id ? 'bg-[var(--c-bg-sub)]' : 'hover:bg-[var(--c-bg-sub)]',
                  ].join(' ')}
                >
          ...
          {completedCount > 0 && (
            <div className="px-3 py-3" style={{ borderTop: '0.5px solid var(--c-border-console)' }}>
              <button
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          ...
          >
            <div className="flex items-center gap-2">
              {selected ? (
                ...
                <h2 className="text-sm font-medium text-[var(--c-text-heading)]">Operations</h2>
              )}
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="flex-1 overflow-hidden p-4">
            {selected ? (
              <div className="h-full overflow-y-auto rounded-md border border-[var(--c-border-console)] bg-[var(--c-bg-page)] p-3">
              ...
              <div className="flex h-full items-center justify-center">
                <p className="text-xs text-[var(--c-text-muted)]">Select an operation to view logs</p>
              </div>
            )}
          </div>
        </div>
