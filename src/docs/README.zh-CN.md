...
- 数据库：兼容 SQLite + PostgreSQL（本地部署 + SaaS）
- Redis：后期引入（缓存、限流、队列）
...
建议采用 monorepo 结构：既能前后端分离部署，也便于统一管理与复用：

- `apps/web/`：用户侧 Web
- `apps/console/`：后台管理
- `services/api/`：后端 API（FastAPI）
- `services/worker/`：异步任务（索引、入库、长文档处理）
- `packages/api-client/`：从 OpenAPI 生成的 TS client/types
- `packages/ui/`：共享 UI 组件
- `packages/shared/`：共享工具与跨端类型
