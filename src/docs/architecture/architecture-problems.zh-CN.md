...
## 八、Phase 1-4 完成后的状态更新（截至 R44）

以下问题已通过 Phase 1-4 重构解决：

| 原始问题 | 解决方式 | 对应 R 号 |
|---|---|---|
| 没有独立 Gateway 层 | 独立 Go reverse proxy + compose service | R40 |
| `users` 表没有 email | migration 00012 | R10 |
| `run_events` 行级锁热点 | 改用 `run_events_seq_global` sequence | R13 |
| MCP stdio 进程绑定 | `mcp_configs` 入库 + Remote HTTP 传输 | R34 |
| Worker 调度盲目 | `worker_registrations` + `jobs.worker_tags` | R36 |
| 没有 Redis | compose.yaml + go-redis，限流/心跳/缓存/run 计数 | R20 |
| `runs` 缺生命周期字段 | migration 00013，Worker 终态同步更新 | R12 + R30 |
| `runs.status` 不可信 | Worker 写终态事件时同事务 UPDATE | R30 |
| `messages` 不支持多模态 | `content_json JSONB` 字段 (migration 00016) | R14 |
| 没有软删除 | `deleted_at` 字段 + WHERE 条件 | R15 |
| SSE 双重延迟 | Worker 批提交降至 50ms + API LISTEN/NOTIFY 驱动 | R31 |
| Skills 绑定文件系统 | `skills` 表入库，文件系统作为 fallback | R35 |
| LLM 凭证锁死 env var | `llm_credentials` + `secrets` 表，DB 优先 env fallback | R33 |
| 审计日志缺 IP | migration 00018，中间件注入 IP/UA | R16 |
| 没有 secrets 统一管理 | `secrets` 表 + AES-256-GCM 加密 | R32 |
| 没有 API Key 管理 | `api_keys` 表 + Gateway 鉴权路径 | R43 |
| 没有并发 Run 限制 | Redis counter `arkloop:org:active_runs:{org_id}` | R44 |
| 没有 Rate Limiting | Gateway Redis token bucket | R41 |
| 没有 IP 过滤 | Gateway `ip_rules` 中间件 | R42 |
| orgs 字段不完整 | migration 00014，owner/status/settings | R11 |

---

## 九、新发现的扩展性盲点（Phase 1-4 未覆盖）

以下问题在原始审计和 Roadmap Phase 1-6 中均未被覆盖，但在 5000+ 并发场景下会成为瓶颈。

### 9.1 没有数据库连接池代理（PgBouncer）-- P1

**现象**：API 和 Worker 各自持有 `pgxpool` 连接池，每个实例占 5-25 个 PostgreSQL 连接。PostgreSQL 默认上限 100-200 连接，调优后最高约 5000-10000。

**影响**：
- 50 个 Worker 实例 x 25 连接 = 1,250 连接，接近默认上限
- 100+ Worker 实例时 PostgreSQL 连接耗尽，新请求直接失败
- API 多副本部署同样消耗连接，两者叠加更快打满

**对标**：所有生产级 PostgreSQL 部署（包括 Supabase、Neon、RDS）都在应用和数据库之间放 PgBouncer（transaction mode），将数千应用连接复用为几百个实际数据库连接。

**建议**：compose.yaml 引入 PgBouncer service，API/Worker 的 DSN 指向 PgBouncer 而非直连 PostgreSQL。改动量极小（一个 service + DSN 切换），但解锁 Worker 水平扩展的上限从"连接数"变为"PG 写吞吐"。

### 9.2 run_events 无月分区 -- P1

**现象**：`run_events` 是系统写入量最大的表。每次 LLM 流式输出约产生 100-300 行 delta 事件。

**影响**：
- 1 万用户 x 每天 10 次对话 = 每天 1000-3000 万行
- 半年后表行数超过 10 亿，索引扫描性能退化到不可用
- 旧数据没有归档机制，磁盘持续增长

**当前状态**：Roadmap 第 9 节明确写了"Phase 3 之后观察再做"，但一直没有执行。R13 解决了行锁热点（sequence），但分区问题独立于序号分配。

**建议**：创建 `run_events` 按 `ts` 的 RANGE 月分区。当前月 + 下月预创建，旧分区归档到对象存储后 DETACH。

### 9.3 SSE 多实例广播缺失 -- P2

**现象**：当前 API SSE handler 通过 `LISTEN run_events:{run_id}` 接收 Worker 的 pg_notify 信号。这在单 API 实例时工作正常。

**影响**：
- 多个 API 实例部署时，用户的 SSE 连接可能连到 API-A，但只有 API-B 在 LISTEN 该 run 的 channel
- pg_notify 只广播到当前 PostgreSQL 连接，不跨进程
- 结果：部分用户的 SSE 推送丢失或严重延迟（退化为心跳轮询）

**当前状态**：architecture-design-v2 第五节已设计了 Redis Pub/Sub 跨实例广播方案，但代码未实现。当前因为只有单 API 实例所以问题未暴露。

**建议**：Worker commit 事件后同时发 `pg_notify` 和 Redis Pub/Sub。API 的 SSE handler 订阅 Redis channel 作为主信号源，pg_notify 作为同实例快速路径。

### 9.4 没有 PostgreSQL 只读副本配置 -- P2

**现象**：所有读写请求打在同一个 PostgreSQL 实例上。

**影响**：
- Console 审计查询、run_events 历史回放、threads/messages 列表等读密集操作与写操作争抢同一个数据库
- 5000+ 用户时读请求延迟上升，影响 SSE 推送的实时性

**建议**：生产环境配置一个只读副本（streaming replication），读密集的查询（审计、历史、列表）路由到只读副本。开发环境不需要。

### 9.5 Worker 崩溃时 Redis 并发计数器泄漏 -- P1

**现象**：R44 的并发 Run 限制使用 Redis counter `arkloop:org:active_runs:{org_id}`。创建 Run 时 INCR，终态时 DECR。如果 Worker 崩溃（进程被 OOM kill、节点故障），DECR 永远不会执行。

**影响**：
- 计数器永久偏高，org 的并发额度被"幽灵 run"占用
- 极端情况下 org 被完全锁死，无法创建新 run

**当前状态**：Roadmap R44 提到了"定期从数据库重新同步"，但代码中未实现该同步机制。

**建议**：后台定时任务（每 60 秒）扫描 `runs` 表中 `status='running'` 且 `updated_at < now() - interval '5 minutes'` 的 run，将其标记为 `failed`，并将 Redis counter 重置为 `SELECT COUNT(*) FROM runs WHERE org_id=$1 AND status='running'`。
