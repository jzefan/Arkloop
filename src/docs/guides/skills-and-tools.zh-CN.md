...
相关测试建议见：`src/docs/specs/testing-and-pytest.zh-CN.md`。

## 8. 当前实现（v1）

Phase 1 先落一个最小可跑版本，重点是“可加载、可注入、可约束、可测试”，不做技能市场与租户自定义。

- 目录：内置 skills 放在 `src/skills/<skill_id>/`，每个 skill 必须包含 `skill.yaml` + `prompt.md`。
- `skill.yaml` 必填字段：`id`、`version`、`title`。
