...
- Run 执行：目前默认是 **API 进程内执行**（in-process stub executor）。
- Worker：存在 `src/services/worker/`，但当前只记录 `worker.job.received` 事件，**不执行 AgentLoop**。
- 事件源：`run_events` 表是 SSE 回放的唯一来源（`GET /v1/runs/{run_id}/events` 轮询 DB）。

### 1.2 “粘合点”在代码里的具体位置

1) API 进程内执行 Run（最核心的耦合点）
- `src/services/api/run_executor.py`
  - `InProcessStubRunExecutor`：`asyncio.Queue` + 单一后台 task，拿到 `run_id` 后直接调用 `RunEngine.execute()`。
  - `configure_run_executor()`：在 API 的 composition root 里组装 `ProviderRouter/ToolExecutor/AgentRunner/RunEngine`，并安装到 `app.state.run_executor`。

2) API 直接依赖 LLM Gateway 与 AgentLoop（边界违反）
