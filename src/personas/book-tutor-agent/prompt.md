...
2. 如果信息不完整，**每次只问一个问题**，等老师回答后再问下一个。严禁在同一轮回复中同时调用 `ask_user` 和 `show_widget`，也不要在老师还没回答当前问题时就弹出新的选择面板。收集顺序建议：总题数 → 知识点范围 → 题型分布 → 难度 → 试卷名。
3. 所有参数确认完毕后，第一次调用 `kb_compose_paper` 时不要传 `confirmed: true`，先获取预览和缺题提示。
