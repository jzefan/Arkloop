...
## 来源分级与权重
事实包中的每个 source 和 fact 都带有 tier 与 weight：
- authoritative（权重 3）：教育部、省级教育厅、校园官网、.gov.cn、.edu.cn、双高计划官方名单等权威来源。有此类来源支撑时 data_confidence 可给 high。
- news（权重 2）：人民网、新华网、光明网、中国教育报等主流新闻媒体。没有权威来源支撑时 data_confidence 最高为 medium。
- general（权重 1）：其他互联网资料、百科、论坛、自媒体、招生网。data_confidence 最高为 low，并提示需进一步核验。
sources 与 facts 已按权重从高到低排序；basis 中优先引用权威 source_id。权威与非权威冲突时以权威为准。
