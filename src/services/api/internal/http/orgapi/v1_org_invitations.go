...
func orgInvitationEntry(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if err != nil {
			httpkit.WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "invalid invitation id", traceID, nil)
			return
		}
...
func createOrgInvitation(
	...
	if !httpkit.RequirePerm(actor, auth.PermOrgMembersInvite, w, traceID) {
		return
	}
	...
	if orgRepo != nil {
		...
		if err != nil || org == nil {
			httpkit.WriteError(w, nethttp.StatusNotFound, "orgs.not_found", "org not found", traceID, nil)
			return
		}
		if org.Type != "workspace" {
			httpkit.WriteError(w, nethttp.StatusBadRequest, "orgs.personal_not_invitable", "cannot invite members to a personal org", traceID, nil)
			return
		}
	}
	...
	if req.Email == "" || len(req.Email) > 254 {
		httpkit.WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "invalid email", traceID, nil)
		return
	}
	...
	if req.Role != "member" && req.Role != "owner" {
		httpkit.WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "role must be member or owner", traceID, nil)
		return
	}
	...
	httpkit.WriteJSON(w, traceID, nethttp.StatusCreated, createOrgInvitationResponse{
...
func listOrgInvitations(
	...
	if !httpkit.RequirePerm(actor, auth.PermOrgMembersList, w, traceID) {
		return
	}
...
func acceptOrgInvitation(
	...
	if len(token) != 64 {
		httpkit.WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "invalid token format", traceID, nil)
		return
	}
	for _, c := range token {
		if !((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f')) {
	...
	if inv == nil {
		httpkit.WriteError(w, nethttp.StatusNotFound, "invitations.not_found", "invitation not found", traceID, nil)
		return
	}
	if inv.AcceptedAt != nil {
		httpkit.WriteError(w, nethttp.StatusConflict, "invitations.already_accepted", "invitation already accepted", traceID, nil)
		return
	}
	if time.Now().UTC().After(inv.ExpiresAt) {
		httpkit.WriteError(w, nethttp.StatusGone, "invitations.expired", "invitation has expired", traceID, nil)
		return
	}

	if user.Email == nil || !strings.EqualFold(*user.Email, inv.Email) {
		httpkit.WriteError(w, nethttp.StatusForbidden, "invitations.email_mismatch", "invitation was sent to a different email", traceID, nil)
		return
	}
	...
	if alreadyMember {
		httpkit.WriteError(w, nethttp.StatusConflict, "invitations.already_member", "user is already a member of this org", traceID, nil)
		return
	}
	...
	if !accepted {
		// 并发场景：邀请在检查后被撤销或已被接受
		httpkit.WriteError(w, nethttp.StatusConflict, "invitations.already_accepted", "invitation no longer available", traceID, nil)
		return
	}
...
func revokeOrgInvitation(
	...
	if !httpkit.RequirePerm(actor, auth.PermOrgMembersRevoke, w, traceID) {
		return
	}
