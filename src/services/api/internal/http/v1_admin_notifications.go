...
func adminBroadcastEntry(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if err != nil {
			WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "invalid broadcast id", traceID, nil)
			return
		}
...
func createBroadcast(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if req.Type == "" {
			WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "type is required", traceID, nil)
			return
		}
		if req.Title == "" {
			WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "title is required", traceID, nil)
			return
		}
		...
		if req.Target != "" && req.Target != "all" {
			...
			if err != nil {
				WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "target must be 'all' or a valid org_id", traceID, nil)
				return
			}
			targetType = "org"
			targetID = &parsed
		...
		if err != nil {
			WriteError(w, nethttp.StatusInternalServerError, "internal.error", "failed to create broadcast", traceID, nil)
			return
		}
		...
		writeJSON(w, traceID, nethttp.StatusAccepted, toBroadcastResponse(broadcast))
	}
}
...
func listBroadcasts(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if err != nil {
			WriteError(w, nethttp.StatusInternalServerError, "internal.error", "failed to list broadcasts", traceID, nil)
			return
		}
...
func getBroadcast(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request, broadcastID uuid.UUID) {
		...
		if err != nil {
			...
			WriteError(w, nethttp.StatusInternalServerError, "internal.error", "failed to get broadcast", traceID, nil)
			return
		}
