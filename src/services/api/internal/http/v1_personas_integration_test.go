...
func TestSelectablePersonasEffectiveForMemberUser(t *testing.T) {
	...
	if _, err := personasRepo.CreateInScope(ctx, orgID, data.PersonaScopeOrg, "normal", "1", "Org Normal", nil, "org normal prompt", nil, nil, json.RawMessage(`{"temperature":0.2}`), nil, nil, strPtrPersonaLocal("org^normal"), "high", "system_prompt", "agent.simple", nil); err != nil {
		t.Fatalf("create org normal: %v", err)
	}
	if _, err := personasRepo.CreateInScope(ctx, orgID, data.PersonaScopeOrg, "org-custom", "1", "Org Custom", nil, "org custom prompt", nil, nil, json.RawMessage(`{"temperature":0.2}`), nil, nil, strPtrPersonaLocal("org^custom"), "high", "system_prompt", "agent.simple", nil); err != nil {
		t.Fatalf("create org custom: %v", err)
	}
	inactive, err := personasRepo.CreateInScope(ctx, orgID, data.PersonaScopeOrg, "extended-search", "1", "Org Search Inactive", nil, "org inactive search prompt", nil, nil, json.RawMessage(`{"temperature":0.2}`), nil, nil, strPtrPersonaLocal("org^search"), "high", "system_prompt", "agent.simple", nil)
	if err != nil {
