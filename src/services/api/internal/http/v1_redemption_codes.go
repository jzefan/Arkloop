...
func adminRedemptionCodesBatch(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if req.Count <= 0 || req.Count > 500 {
			WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "count must be between 1 and 500", traceID, nil)
			return
		}
		if req.Type != "credit" && req.Type != "feature" {
			WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "type must be credit or feature", traceID, nil)
			return
		}
		if strings.TrimSpace(req.Value) == "" {
			WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "value is required", traceID, nil)
			return
		}
		if req.Type == "credit" {
			if _, err := strconv.ParseInt(req.Value, 10, 64); err != nil {
				WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "value must be a valid integer for credit type", traceID, nil)
				return
			}
		}
		if req.MaxUses <= 0 {
		...
		if req.ExpiresAt != nil {
			...
			if err != nil {
				WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "expires_at must be RFC3339 format", traceID, nil)
				return
			}
			expiresAt = &t
		}
		...
		writeJSON(w, traceID, nethttp.StatusCreated, codes)
	}
}
...
func patchAdminRedemptionCode(
	...
	if body.IsActive == nil {
		WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "is_active is required", traceID, nil)
		return
	}
	if *body.IsActive {
		WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "only deactivation is supported", traceID, nil)
		return
	}
	...
	if rc == nil {
		WriteError(w, nethttp.StatusNotFound, "redemption_codes.not_found", "redemption code not found", traceID, nil)
		return
	}

	writeJSON(w, traceID, nethttp.StatusOK, toRedemptionCodeResponse(*rc))
}
...
func meRedeem(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if code == "" {
			WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "code is required", traceID, nil)
			return
		}
		...
		if rc == nil {
			WriteError(w, nethttp.StatusNotFound, "redemption_codes.not_found", "invalid redemption code", traceID, nil)
			return
		}

		if !rc.IsActive {
			WriteError(w, nethttp.StatusConflict, "redemption_codes.inactive", "redemption code is inactive", traceID, nil)
			return
		}
		if rc.ExpiresAt != nil && rc.ExpiresAt.Before(time.Now()) {
			WriteError(w, nethttp.StatusConflict, "redemption_codes.expired", "redemption code has expired", traceID, nil)
			return
		}
		if rc.UseCount >= rc.MaxUses {
			WriteError(w, nethttp.StatusConflict, "redemption_codes.exhausted", "redemption code has been fully used", traceID, nil)
			return
		}
		...
		if redeemed {
			WriteError(w, nethttp.StatusConflict, "redemption_codes.already_redeemed", "already redeemed this code", traceID, nil)
			return
		}
		...
		if !incremented {
			WriteError(w, nethttp.StatusConflict, "redemption_codes.exhausted", "redemption code is no longer available", traceID, nil)
			return
		}
		...
		writeJSON(w, traceID, nethttp.StatusOK, redeemResponse{
			Code:  rc.Code,
			Type:  rc.Type,
			Value: rc.Value,
		})
