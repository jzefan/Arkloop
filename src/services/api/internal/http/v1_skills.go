...
func skillPackagesEntry(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if repo == nil {
			WriteError(w, nethttp.StatusServiceUnavailable, "skills.not_configured", "skills not configured", traceID, nil)
			return
		}
		...
		if !requirePerm(actor, auth.PermDataPersonasRead, w, traceID) {
			return
		}
		...
		writeJSON(w, traceID, nethttp.StatusOK, map[string]any{"items": toSkillPackageResponses(items)})
	}
}
...
func adminSkillPackagesEntry(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
			WriteError(w, nethttp.StatusBadRequest, "skills.invalid_request", "invalid JSON body", traceID, nil)
			return
		}
		...
		if err != nil {
			...
			if errors.As(err, &conflict) {
				WriteError(w, nethttp.StatusConflict, "skills.conflict", "skill package already exists", traceID, nil)
				return
			}
			WriteError(w, nethttp.StatusBadRequest, "skills.invalid_manifest", err.Error(), traceID, nil)
			return
		}
		writeJSON(w, traceID, nethttp.StatusCreated, toSkillPackageResponse(item))
	}
}
...
func skillPackageEntry(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if item == nil || !item.IsActive {
			WriteError(w, nethttp.StatusNotFound, "skills.not_found", "skill package not found", traceID, nil)
			return
		}
		writeJSON(w, traceID, nethttp.StatusOK, toSkillPackageResponse(*item))
	}
}
...
func profileSkillsEntry(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		writeJSON(w, traceID, nethttp.StatusOK, map[string]any{"items": items})
	}
}
...
func profileSkillsInstallEntry(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		writeJSON(w, traceID, nethttp.StatusCreated, map[string]any{"skill_key": req.SkillKey, "version": req.Version})
	}
}
...
func profileSkillEntry(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if used {
			WriteError(w, nethttp.StatusConflict, "skills.in_use", "skill is enabled by a workspace", traceID, nil)
			return
		}
...
func workspaceSkillsEntry(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if registry == nil || registry.OrgID != actor.OrgID || registry.OwnerUserID == nil || *registry.OwnerUserID != actor.UserID {
			WriteError(w, nethttp.StatusNotFound, "workspaces.not_found", "workspace not found", traceID, nil)
			return
		}
		profileRef := sharedenvironmentref.BuildProfileRef(actor.OrgID, &actor.UserID)
		switch r.Method {
		...
		case nethttp.MethodPut:
			...
			for _, item := range req.Skills {
				...
				if !installed {
					WriteError(w, nethttp.StatusBadRequest, "skills.not_installed", "skill is not installed for this profile", traceID, nil)
					return
				}
...
func parseSkillPackagePath(w nethttp.ResponseWriter, traceID string, raw string, prefix string) (string, string, bool) {
	...
	if len(parts) != 2 || strings.TrimSpace(parts[0]) == "" || strings.TrimSpace(parts[1]) == "" {
		WriteError(w, nethttp.StatusBadRequest, "skills.invalid_path", "invalid skill path", traceID, nil)
		return "", "", false
	}
...
func writeSkillStoreError(w nethttp.ResponseWriter, traceID string, err error) {
	if objectstore.IsNotFound(err) {
		WriteError(w, nethttp.StatusNotFound, "skills.bundle_not_found", "skill package artifacts not found", traceID, nil)
		return
	}
	WriteError(w, nethttp.StatusBadRequest, "skills.invalid_manifest", err.Error(), traceID, nil)
}
