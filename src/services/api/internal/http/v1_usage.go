...
func resolveAdminUsageActor(
	...
	return requirePerm(actor, auth.PermPlatformAdmin, w, traceID)
}
...
func parseDateRange(w nethttp.ResponseWriter, r *nethttp.Request, traceID string) (time.Time, time.Time, bool) {
	...
	if startStr == "" || endStr == "" {
		WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "start and end are required (YYYY-MM-DD)", traceID, nil)
		return time.Time{}, time.Time{}, false
	}

	startDate, err := time.Parse("2006-01-02", startStr)
	if err != nil {
		WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "invalid start date", traceID, nil)
		return time.Time{}, time.Time{}, false
	}

	endDate, err := time.Parse("2006-01-02", endStr)
	if err != nil {
		WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "invalid end date", traceID, nil)
		return time.Time{}, time.Time{}, false
	}
