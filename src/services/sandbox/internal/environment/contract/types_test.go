...
func TestObjectKeys(t *testing.T) {
	...
	if got := ProfileLatestKey("pref_1"); got != "profiles/pref_1/latest.json" {
		t.Fatalf("unexpected profile latest key: %s", got)
	}
	...
	if got := WorkspaceLatestKey("ws_1"); got != "workspaces/ws_1/latest.json" {
		t.Fatalf("unexpected workspace latest key: %s", got)
	}
