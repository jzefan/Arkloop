...
func ComposeNativeEngine(ctx context.Context, pool *pgxpool.Pool, directPool *pgxpool.Pool, rdb *redis.Client, cfg Config, execRegistry pipeline.AgentExecutorBuilder, jobQueue queue.JobQueue) (*runengine.EngineV1, error) {
	...
	allLlmSpecs, artifactToolsRegistered, err := registerStoredArtifactTools(toolRegistry, executors, allLlmSpecs, artifactStore, dynamicSandboxExec, pool, configResolver, routingLoader)
	if err != nil {
		return nil, err
	}
	if artifactToolsRegistered {
		slog.InfoContext(ctx, "stored artifact tools registered", "tools", []string{"create_artifact", "document_write", "markdown_to_pdf", "image_generate", "video_generate", "video_concat", "frame_extract"})
	}
