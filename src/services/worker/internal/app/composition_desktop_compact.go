...
func resolveDesktopLLMRetry(ctx context.Context, db data.DesktopDB) (int, int) {
	registry := sharedconfig.DefaultRegistry()
	if db == nil {
		return desktopResolvePositiveInt(ctx, nil, registry, "llm.retry.max_attempts", sharedconfig.Scope{}, 5),
	...
	scope := sharedconfig.Scope{}
	return desktopResolvePositiveInt(ctx, resolver, registry, "llm.retry.max_attempts", scope, 5),
