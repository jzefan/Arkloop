...
func TestGeminiSDKGateway_GenerateImageUsesZenMuxVertexGenerateImages(t *testing.T) {
	t.Setenv("ARKLOOP_OUTBOUND_ALLOW_LOOPBACK_HTTP", "true")
	png := base64.StdEncoding.EncodeToString([]byte("\x89PNG\r\n\x1a\nimage"))
	var captured map[string]any
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/publishers/openai/models/gpt-image-2:predict" {
			t.Fatalf("unexpected path: %s", r.URL.String())
		}
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"predictions":[{"bytesBase64Encoded":"` + png + `","mimeType":"image/png","prompt":"enhanced prompt"}]}`))
	}))
	defer server.Close()
	...
	cfg := ResolvedGatewayConfig{
		...
		Gemini:       &GeminiProtocolConfig{APIVersion: "v1"},
	}
	image, err := GenerateImageWithResolvedConfig(context.Background(), cfg, ImageGenerationRequest{Prompt: "draw", Size: "16:9", OutputFormat: "png"})
	if err != nil {
		t.Fatalf("GenerateImageWithResolvedConfig: %v", err)
	}
	if image.ProviderKind != "zenmux" || image.Model != "openai/gpt-image-2" || image.MimeType != "image/png" || len(image.Bytes) == 0 {
		t.Fatalf("unexpected image: %#v", image)
	}
	if image.RevisedPrompt != "enhanced prompt" {
		t.Fatalf("unexpected revised prompt: %q", image.RevisedPrompt)
	}
	instances := captured["instances"].([]any)
	if instances[0].(map[string]any)["prompt"] != "draw" {
		t.Fatalf("unexpected request body: %#v", captured)
	}
	params := captured["parameters"].(map[string]any)
	if params["aspectRatio"] != "16:9" || params["outputOptions"].(map[string]any)["mimeType"] != "image/png" {
		t.Fatalf("unexpected parameters: %#v", params)
	}
}

func TestGeminiSDKGateway_ZenMuxVertexImageRetriesTransientStatus(t *testing.T) {
	t.Setenv("ARKLOOP_OUTBOUND_ALLOW_LOOPBACK_HTTP", "true")
	png := base64.StdEncoding.EncodeToString([]byte("\x89PNG\r\n\x1a\nimage"))
	attempts := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"predictions":[{"bytesBase64Encoded":"` + png + `","mimeType":"image/png"}]}`))
	}))
	defer server.Close()

	gateway := NewGeminiGatewaySDK(GeminiGatewayConfig{
		Transport: TransportConfig{APIKey: "key", BaseURL: server.URL},
		Protocol:  GeminiProtocolConfig{APIVersion: "v1"},
	}).(*geminiSDKGateway)
	image, err := gateway.generateImageWithZenMuxVertexPredict(context.Background(), "openai/gpt-image-2", ImageGenerationRequest{Prompt: "draw"})
	if err != nil {
		t.Fatalf("GenerateImageWithResolvedConfig: %v", err)
	}
	if attempts != 2 {
		t.Fatalf("expected retry after 502, got %d attempts", attempts)
	}
	if image.ProviderKind != "zenmux" || len(image.Bytes) == 0 {
		t.Fatalf("unexpected image: %#v", image)
	}
}
