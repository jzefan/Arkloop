...
// 当 skill 定义了 tool_allowlist 时，进一步收窄到 skill 允许的工具集。
func NewToolBuildMiddleware() RunMiddleware {
	return func(ctx context.Context, rc *RunContext, next RunHandler) error {
		effectiveAllowlist := CopyStringSet(rc.AllowlistSet)

		if rc.SkillDefinition != nil && len(rc.SkillDefinition.ToolAllowlist) > 0 {
			effectiveAllowlist = make(map[string]struct{})
			for _, name := range rc.SkillDefinition.ToolAllowlist {
