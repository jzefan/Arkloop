...
async def _roundtrip(sqlalchemy_url: str) -> None:
    ...
    try:
        async with database.sessionmaker() as session:
            ...
            assert await org_repo.get_by_id(org.id) == org
            assert await org_repo.get_by_slug(org.slug) == org
            assert await user_repo.get_by_id(user.id) == user
            assert (
                await membership_repo.get_by_org_and_user(org_id=org.id, user_id=user.id)
                == membership
            )
    finally:
...
def test_identity_roundtrip_via_repositories(monkeypatch) -> None:
    ...
    database = f"arkloop_identity_{uuid.uuid4().hex}"
