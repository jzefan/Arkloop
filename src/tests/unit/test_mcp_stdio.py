...
from packages.mcp.executor import ERROR_CLASS_MCP_DISCONNECTED, ERROR_CLASS_MCP_TIMEOUT
...
def test_mcp_stdio_executor_timeout_returns_clear_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def _run():
        registration = await discover_mcp_tools(config=McpConfig(servers=(server,)))
        tool_name = registration.agent_specs[0].name
