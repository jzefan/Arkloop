...
def main() -> int:
    ...
    for index, task in enumerate(selected, start=1):
        print(f"[{index}/{len(selected)}] {task.task_id} {task.domain} {task.level}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model",
        default=os.environ.get("ARKLOOP_MODEL", ""),
...
def build_summary(
    ...
    arkloop_errors = sum(1 for item in results if not item["arkloop_ok"])
    total = len(results)
